"use client"
import { useGetMyImages, useGetPublicCategories } from "@/store/api"
...
export default function DashboardOverviewPage() {
  ...
  const { data: categoriesResponse, isLoading: loadingCats } = useGetPublicCategories()
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className={`grid grid-cols-1 md:grid-cols-2 ${user?.role === "ADMIN" ? "lg:grid-cols-4" : "lg:grid-cols-3"} gap-6 mb-12`}>
        {user?.role === "ADMIN" && (
          <div className="bg-surface-card p-6 rounded-[16px]">
            <p className="text-body-strong text-mute mb-2">Categories</p>
      ...
      <div className={`grid grid-cols-1 ${user?.role === "ADMIN" ? "sm:grid-cols-2" : ""} gap-4`}>
        <Link href="/dashboard/upload" className="flex items-center gap-4 p-6 bg-surface-card rounded-[16px] hover:bg-secondary-pressed transition-colors group">
          <div className="w-12 h-12 rounded-full bg-primary flex items-center justify-center text-primary-foreground group-hover:scale-105 transition-transform">
